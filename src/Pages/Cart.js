...
function incQty(item){
    if(item.product.stock == item.qty ||  item.product.stock < 1)
    {
        return;
    }
...
<div class="row">
    <div class="col-4 col-lg-3">
        <img src={item.product.images[0].url} alt="chocolate" height="90" width="115"/>
    </div>
...
<button id="checkout_btn" class="btn btn-primary btn-block" onClick={placeOrder}>Place Order</button>
    ...
    </Fragment> : (!comp ? <h2 className='mt-5'> Your cart is Empty...Please add anything...!</h2> : <Fragment>
        <h2>Order Completed!!</h2>
        <p>Your orders has been placed 👍</p>
        </Fragment>)
